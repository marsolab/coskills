...
3. Wraps every individual skill into a self-contained `.zip` archive.
...
└── skills/
    ...
    ├── vercel-react-native-skills.zip
    ├── vercel-react-view-transitions.zip
    └── web-design-guidelines.zip
```

Each zip contains the skill directory at its root (e.g. `deploy-to-vercel/SKILL.md`), so a coworker can drop it into their own `.claude/skills/` (or any agent-skills compatible folder) and start using it.
...
The zip is built in pure Node with `zlib`, so there are no native dependencies and no need for the system `zip` utility.
